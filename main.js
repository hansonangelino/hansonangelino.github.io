...
var jAlbumController = (function () {
	...
	if(!jAlbumGlobals1679471386669.credits) $jAlbum('#jAlbum-footer .center').hide(); // hide credits in footer if disabled

	/** go back also on background click possible **/
	$jAlbum("#Responsive" + jAlbumGlobals1679471386669.uniqueId + ", .jAlbum #jAlbum-footer, .jAlbum #jAlbum-footer .center, .jAlbum #jAlbum-footer .left, .jAlbum #jAlbum-footer p, .jAlbum #jAlbum-content, .jAlbum #print").click(function(event) {
	...
	function getWidth(){
		return $jAlbum("#Responsive" + jAlbumGlobals1679471386669.uniqueId).width();
	}
	...
	function getMaxLetterParam(){
		return (jAlbumGlobals1679471386669.imgBoxSize / 25) * 3 + (16 - jAlbumGlobals1679471386669.textSize) * 2 - 8;
	}

	function getImgBoxPadding(){
		return jAlbumGlobals1679471386669.mobile ? 15 : 25;
	}

	function getBoxSize(){
		return jAlbumGlobals1679471386669.mobile ? jAlbumGlobals1679471386669.imgBoxSize - 25 : jAlbumGlobals1679471386669.imgBoxSize;
	}

	function getNumImgsInRow(innerWidth){
	    var number = Math.round((innerWidth)/BOX_SIZE);
	    return (number < 1) ? 1 : (number > jAlbumGlobals1679471386669.maxImgInRow) ? jAlbumGlobals1679471386669.maxImgInRow : number;
	}
	...
	function setHashPathAndSlide(currentSlide){
		...
		if(currentPath == "" && jAlbumGlobals1679471386669.online) history.pushState('', document.title, window.location.pathname);
	...
	function preloadCurrSlides(){
		for(var i = 0; i < slides.length; i++){
			preloadImage(jAlbumGlobals1679471386669.jAlbumURL + currentPath + slides[i].path);
		}
	}
	...
	function preloadBeforeAndAfter(index, number){ // number desides how many images should be loaded in both directions
		...
		for(var i = start; i < end; i++){
			if (slides[i].category == 'image') {
				jAlbumLibrary.preloadImg(jAlbumGlobals1679471386669.jAlbumURL + currentPath + slides[i].path);
			}
		}
	}
	...
	function getAlbumData(path){
		var obj = jAlbumGlobals1679471386669.dataTree; // jAlbum dataTree
	...
	function arrange() {
		...
		for(var i = 0; i < 2; i++){
			...
			$jAlbum("#Responsive" + jAlbumGlobals1679471386669.uniqueId + " #jAlbum-content").css("padding", '' + topBottomPadding + 'px 2.5%');
	...
	function initFolder(index, json){
			...
			if(numOfImg < 3){ // 0 is the first one
				switch (object.category) {
					...
					case 'image':
						...
						var img = jAlbumLibrary.img("img-folder-" + numOfImg + "-" + index, jAlbumGlobals1679471386669.jAlbumURL + imgPath);
						...
						if(!jAlbumGlobals1679471386669.mobile){
						    img.hover(function(){
						    	mainBox.css({
						    		"-webkit-transform": jAlbumGlobals1679471386669.imgHoverScaleFactor,
							    	"-moz-transform": jAlbumGlobals1679471386669.imgHoverScaleFactor,
							    	"-o-transform": jAlbumGlobals1679471386669.imgHoverScaleFactor,
							    	"transform": jAlbumGlobals1679471386669.imgHoverScaleFactor
						    	});
							}, function(){
								mainBox.css({
		...
		if(numOfImg == 0){
		    ...
		    img.attr("src", jAlbumGlobals1679471386669.jAlbumURL + "res/img/folder.png");
		    img.css("position", "absolute");

			// add style
			if(jAlbumGlobals1679471386669.showFolderName){
				img.css("width", "60%");
				img.css("margin", "20%");
			}else{
				img.css("width", "80%");
		...
		if(jAlbumGlobals1679471386669.showFolderName){
			...
			if(jAlbumGlobals1679471386669.folderImgCount){
	    ...
	    $jAlbum("#Responsive" + jAlbumGlobals1679471386669.uniqueId + " #jAlbum-content").append(coverBox);
	...
	function arrangeFolders(mainBoxSize, imgSize){
		...
		if(jAlbumGlobals1679471386669.folderTitleUp){
		...
		imgSize = jAlbumGlobals1679471386669.showFolderName ? imgSize * 0.5 : imgSize * 0.8;
		...
		$jAlbum.each(folders, function(index, folder) {
			...
			if(folder.width != undefined){
				...
				for(var i = 0; i < 3; i++){
					...
					var img = $jAlbum("#Responsive" + jAlbumGlobals1679471386669.uniqueId + " #img-folder-" + i + "-" + folder.index);
					var x = (i - 1) * - 10;
					var y = (i - 1) * - 10;
					var scaledWidth = 0;
					var scaledHeight = 0;
					...
					img.css("margin", ((mainBoxSize - scaledHeight)/2 - jAlbumGlobals1679471386669.imgBorderSize) + "px " + ((mainBoxSize - scaledWidth)/2 - jAlbumGlobals1679471386669.imgBorderSize) + "px");
				}
			}
		}); 
	}
	...
	function initImage(index, object){
	    ...
	    var img = jAlbumLibrary.img("img-thumb-" + index, jAlbumGlobals1679471386669.jAlbumURL + currentPath + object.thumb.path);
	    ...
	    if(object.category == 'video'){
		    ...
		    video_icon.attr("src", jAlbumGlobals1679471386669.jAlbumURL + "res/img/play.png");
	    ...
	    $jAlbum("#Responsive" + jAlbumGlobals1679471386669.uniqueId + " #jAlbum-content").append(box);
	...
	function arrangeImages(imgBoxSize, imgSize) {
		...
		$jAlbum.each(thumbs, function(index, thumb) {
	    	...
	    	var img = $jAlbum("#Responsive" + jAlbumGlobals1679471386669.uniqueId + " #img-thumb-" + thumb.index);
			...
			img.css("margin", ((imgBoxSize - scaledHeight)/2 - jAlbumGlobals1679471386669.imgBorderSize) + "px " + ((imgBoxSize - scaledWidth)/2 - jAlbumGlobals1679471386669.imgBorderSize) + "px");
	...
	function fullscreen(index){
	    ...
	    var play = $jAlbum("<div/>");
	    var thumbnail = slides[index].category == 'video' ? $jAlbum("<img src='" + jAlbumGlobals1679471386669.jAlbumURL + currentPath + slides[index].videothumb + "'>") : $jAlbum('<img>');
	    var description = $jAlbum("<div/>");

	    // hide description if disabled
	    if(!jAlbumGlobals1679471386669.showComments) description.hide();

	    thumbnail.css({"width": "100%", "height": "100%", "position": "absolute"});

	    if(!jAlbumGlobals1679471386669.mobile) thumbnail.css("opacity", "0");

	    description.unbind();

	    // handle navigation
		var fs = $jAlbum("#fullscreen");
		var em = $jAlbum("#Responsive" + jAlbumGlobals1679471386669.uniqueId)
		...
		switch (slides[index].category) {
			case 'image':
				img = jAlbumLibrary.img("img-fullscreen-" + index, jAlbumGlobals1679471386669.jAlbumURL + path);
				play.hide();
				break;
			case 'video':
				img = jAlbumLibrary.vid("img-fullscreen-" + index, jAlbumGlobals1679471386669.jAlbumURL + path);
				play.show();
				break;
		}
	...
	function arrangeFullscreen(){
		...
		if(tempSlideIndex !== null){
			...
			var imageBoxWidth = innerWidth - jAlbumGlobals1679471386669.slideMarginSize * 2 * innerWidth / 100;
			var imageBoxHeight = innerHeight - jAlbumGlobals1679471386669.slideMarginSize * 2 * innerHeight / 100;
